#!/usr/bin/env node
// scripts/backfill_events.js
// Backfill events for existing matches using SportMonks fixtures
// Usage: node scripts/backfill_events.js [--limit=N] [--batchSize=M] [--delay=ms] [--dry]
...
const dry = !!argv.dry;
const batchSize = Number(argv.batchSize) || 100;
...
async function fetchFixtureEvents(id){
  try{
    ...
    // Normalised events from utils/normaliseFixture already map important fields; we will
    // persist the provider event object list for now (normaliser will also be used on next sync)
    const events = payload.events?.data || payload.events || [];
    return { events, raw: payload };
  }catch(e){
    const code = e?.response?.data?.code;
    if (code === 5001) {
      // include rejected; try again without include
...
async function main(){
  ...
  const q = { $or: [ { 'events': { $exists: false } }, { 'events.0': { $exists: false } } ] };
