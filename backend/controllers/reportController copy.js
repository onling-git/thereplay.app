...
async function generateReportFor(matchId, teamSlug) {
  ...
  try {
    if (team?.id) {
      ...
      // Focus on tweets during actual match time with priority for reporters
      const matchTweets = await Tweet.find({
        ...
        created_at: 1 // Chronological within match
      }).limit(10).lean();
      
      // If no reporter tweets during match, fall back to any tweets during match
      if (!matchTweets || matchTweets.length === 0) {
        console.log('⚠️ No reporter tweets during match, checking all tweets during match time...');
