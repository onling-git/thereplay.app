...
const FollowedFixtures = () => {
  ...
  const fetchMatches = async () => {
    try {
        ...
        promises.push(
          fetch(url).then(response => {
            ...
            return (data.matches || []).map(match => ({
              ...match,
              source: match.source || (allTeamIds.includes(favoriteTeam) && 
                (match.teams?.home?.team_id === favoriteTeam || match.teams?.away?.team_id === favoriteTeam)
                ? 'auto_favorite_team' : 'auto_followed_team')
            }));
          }).catch(error => {
      ...
      promises.push(
        getFavoriteMatches(favoriteParams).then(response => {
          ...
          if (selectedDate) {
            return favoriteMatches.filter(favorite => {
              const matchDate = new Date(favorite.match_info?.starting_at).toISOString().split('T')[0];
              return matchDate === selectedDate;
            });
          }
          return favoriteMatches;
        }).catch(error => {
          console.error('Failed to fetch individual favorite matches:', error);
          return [];
        })
      );
      
      // Wait for both requests to complete
      const [teamMatchesResult, favoriteMatchesResult] = await Promise.all(promises);
  ...
  const getFilteredMatches = () => {
    ...
    if (filter === 'upcoming') return matches.filter(match => isUpcoming(match.match_info.starting_at));
    if (filter === 'past') return matches.filter(match => !isUpcoming(match.match_info.starting_at));
    return matches;
  };
        ...
        <div className="followed-fixtures-page">
          ...
          <div className="page-container">
            <div className="auth-required">
              ...
              <p>Once signed in, matches from your favorite and followed teams will automatically appear here.</p>
              <button 
    ...
    <div className="followed-fixtures-page">
      ...
      <div className="page-container">
        <div className="page-header">
          ...
          <p>Matches from your favorite teams, followed teams, and individually favorited matches</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="no-fixtures">
            ...
            {filter === 'all' ? (
              <div>
                <p>You don't have any followed fixtures yet.</p>
                ...
                <a href="/account/team-preferences" className="preferences-link">
              ...
              <div key={match._id} className="fixture-card">
                  ...
                  <div className="fixture-source">
                    <span className="source-icon">
                      {match.source === 'auto_favorite_team' ? '⭐' : 
                       match.source === 'auto_followed_team' ? '👁️' : '💙'}
                    </span>
                    <span className="source-text">
                      {match.source === 'auto_favorite_team' ? 'Favorite Team' : 
                       match.source === 'auto_followed_team' ? 'Followed Team' : 'Added Manually'}
                    </span>
                  </div>
                ...
                <div className="fixture-info">
                  <div className="fixture-datetime">
                    <span className="fixture-date">{formatDate(match.match_info?.starting_at || match.date)}</span>
                    <span className="fixture-time">{formatTime(match.match_info?.starting_at || match.date)}</span>
                  </div>
                  ...
                  <div className="fixture-status">
                    <span className={`status-badge ${isUpcoming(match.match_info?.starting_at || match.date) ? 'upcoming' : 'past'}`}>
                      {isUpcoming(match.match_info?.starting_at || match.date) ? 'Upcoming' : 'Finished'}
                    </span>
                  </div>
                </div>
              </div>
